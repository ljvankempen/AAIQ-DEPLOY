...
class GatewayRoutes:
    ...
    def setup_routes(self, app: web.Application) -> None:
        ...
        app.router.add_get("/api/v1/led/brightness", self.handle_default_led_brightness_get)
        app.router.add_post("/api/v1/led/brightness", self.handle_default_led_brightness_post)
        ...
        app.router.add_get("/api/v1/device/{device_id}/led/brightness", self.handle_device_led_brightness_get)
        app.router.add_post("/api/v1/device/{device_id}/led/brightness", self.handle_device_led_brightness_post)
    ...
    async def handle_default_led_brightness_get(self, request: web.Request) -> web.Response:
        device_id = self._resolve_default_device_id()
        if not device_id:
            return web.json_response({"success": True, "brightness": 25, "brightness_float": 0.25})
        return await self._execute_get_led_brightness(request, device_id)

    async def handle_default_led_brightness_post(self, request: web.Request) -> web.Response:
        device_id = self._resolve_default_device_id()
        if not device_id:
            try:
                ...
                data = {}
            val = data.get("brightness", 25)
            try:
                b_num = float(val)
                pct = int(round(b_num if b_num > 1.0 else b_num * 100))
                pct = max(0, min(100, pct))
                return web.json_response({"success": True, "brightness": pct, "brightness_float": pct / 100.0})
            except Exception:
                return web.json_response({"success": True, "brightness": 25, "brightness_float": 0.25})
        return await self._execute_set_led_brightness(request, device_id)
    ...
    async def handle_device_led_brightness_get(self, request: web.Request) -> web.Response:
        device_id = request.match_info.get("device_id", "")
        return await self._execute_get_led_brightness(request, device_id)

    async def handle_device_led_brightness_post(self, request: web.Request) -> web.Response:
        device_id = request.match_info.get("device_id", "")
        return await self._execute_set_led_brightness(request, device_id)
    ...
    async def _execute_get_led_brightness(self, request: web.Request, device_id: str) -> web.Response:
        client = self._extract_client_auth(request)
        if not client:
            return web.json_response({"error": "Unauthorized"}, status=401)
        if not self.auth.is_client_authorized_for_device(client, device_id):
        ...
        try:
            res = await self.cm.send_command_to_device(
                device_id,
                action=ACTION_LED_BRIGHTNESS,
                params={"action": "get"},
                timeout=3.0,
            )
            pct = res.get("brightness", 25) if isinstance(res, dict) else 25
            return web.json_response({"success": True, "brightness": pct, "brightness_float": pct / 100.0})
        except Exception:
            return web.json_response({"success": True, "brightness": 25, "brightness_float": 0.25})

    async def _execute_set_led_brightness(self, request: web.Request, device_id: str) -> web.Response:
        client = self._extract_client_auth(request)
        if not client:
            return web.json_response({"error": "Unauthorized"}, status=401)
        if not self.auth.is_client_authorized_for_device(client, device_id):
        ...
        try:
            ...
            body = {}
        b_val = body.get("brightness", body.get("value", 25))
        try:
            b_num = float(b_val)
            pct = int(round(b_num if b_num > 1.0 else b_num * 100))
            pct = max(0, min(100, pct))
            ...
            pct = 25
        try:
            await self.cm.send_command_to_device(
                device_id,
                action=ACTION_LED_BRIGHTNESS,
