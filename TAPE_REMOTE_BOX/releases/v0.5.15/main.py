...
def ota_download_and_verify(download_url, expected_sha256=None, expected_size=None, timeout_s=30, watchdog=None):
    try:
        ...
        # Keep the in-process validation deliberately bounded.  The full image
        # is validated by boot.py after reboot, when the runtime heap is empty.
...
def ota_commit_install(target_version, expected_sha256=None, expected_size=None):
    """Mark a verified download for boot.py to install after reboot.

    Do not load, compile, back up, or replace firmware here: main.py is the
    memory-constrained runtime.  boot.py runs after reboot with a clean heap.
    """
    if not _file_exists(OTA_DOWNLOAD_FILE):
        return False, "DOWNLOAD_FILE_MISSING"

    # boot.py repeats the SHA-256 and size validation before it touches main.py.
    try:
        marker = {
            ...
            "boot_attempts": 0,
            "status": "pending",
            "sha256": str(expected_sha256 or "").strip().lower(),
        ...
        return False, "MARKER_WRITE_FAILED"

    print("[OTA] Verified firmware marked for boot installation. Target version: v%s" % target_version)
    return True, "SUCCESS"
...
class OTAManager:
    ...
    def start_install(self, user_confirmed=False):
        ...
        self._log("Marking verified firmware v%s for boot installation..." % target_ver)
        ok_install, res_install = ota_commit_install(target_ver, expected_sha, expected_size)
        if not ok_install:
            self.state = "error"
        ...
        self._log("Firmware update verified. Initiating system reboot for safe installation...")
        return {
            "success": True,
            "message": "Firmware verified. Installing safely during reboot...",
            "version": target_ver,
            "reboot_pending": True,
        }
...
PWA_MANIFEST = {
    ...
    "icons": [
        ...
        {
            "src": "https://taperc.aaiq.nl/images/icon-192.png",
            "sizes": "192x192",
            "type": "image/png",
            "purpose": "any"
        },
        {
            "src": "https://taperc.aaiq.nl/images/icon-512.png",
            "sizes": "512x512",
            "type": "image/png",
            "purpose": "any"
        },
        {
            "src": "https://taperc.aaiq.nl/images/apple-touch-icon.png",
            "sizes": "180x180",
            "type": "image/png",
            "purpose": "any"
        }
...
SW_JS = """const CACHE_NAME = 'tape-remote-v0.5.15';
const STATIC_ASSETS = ['/', '/remote', '/manifest.json', '/icon.svg'];
